...
namespace CustomVision
{
    public class ImageClassifier
    {
        ...
        //readonly List<string> labs;
        readonly TensorFlowInferenceInterface inferenceInterface;
        //readonly TensorFlowInferenceInterface inferenceInterface2;

        public ImageClassifier()
        {
            var assets = Application.Context.Assets;
			inferenceInterface = new TensorFlowInferenceInterface(assets, "model.pb");
            //inferenceInterface2 = new TensorFlowInferenceInterface(assets, "mods.pb");
           ...
           /* using (var sr = new StreamReader(assets.Open("labs.txt")))
            {
                var content = sr.ReadToEnd();
                labs = content.Split('\n').Select(s => s.Trim()).Where(s => !string.IsNullOrEmpty(s)).ToList();
            }*/
       ...
       /* public string RecognizeImage2(Bitmap bitmap)
        {
            ...
            var outputs = new float[labs.Count];
            ...
            for (var i = 0; i < outputs.Length; ++i)
                results.Add(Tuple.Create(outputs[i], labs[i]));
